...
# Make same type of variable importance plot as in paper
...
varImportance<- varImportance[c(1:6),]
varImportance$var_categ <- c(1,2,1,2,2,2) # random var category
#plot
...
## Standard Deviation of Gradient Boosting Variable Importance Rankings
```{r}
suppressWarnings({
set.seed(1)
maxIter<-500

#instantiate model with same hyperparameters as best model
FinalNumTrees<-boost[["bestTune"]][1]
FinalShrinkage<-boost[["bestTune"]][3]
Finalminobs<-boost[["bestTune"]][4]
FinalInteractionDepth<-2

#grid only has 1 combination
tunegrid <- expand.grid(n.trees=as.numeric(FinalNumTrees),interaction.depth=FinalInteractionDepth,shrinkage=as.numeric(FinalShrinkage),n.minobsinnode=as.numeric(Finalminobs))
control <- trainControl(method='cv', number=5)

#Repeat 100 times:
#1) randomly leave out 5% of the data
#2) perform 5-fold cv like normal on the 95% of data we kept
#3) get variable importance rankings

#Perform 1st iteration of this algorithm / initialize the containers
keep<-sample(nrow(allData),76) #step 1
allData_temp <- allData[keep,]
boost<- train(Y~.,data=allData_temp, method='gbm',tuneGrid=tunegrid,trControl=control,verbose=F)#step 2
varImportance<-varImp(boost) #step 3
varImportance<- as.data.frame(varImportance$importance)
varImportance$varnames <- rownames(varImportance) 
rownames(varImportance) <- NULL  
varImportance<- varImportance[order(-varImportance$Overall),]


#Now that we have containers to append results to, continue with other iterations
for(i in 1:(maxIter-1)){
  keep<-sample(nrow(allData),76) #step 1
  allData_temp <- allData[keep,]
  boost<- train(Y~.,data=allData_temp, method='gbm',tuneGrid=tunegrid, trControl=control, verbose=F) #step 2
  varImportance_repetition<-varImp(boost) #step 3
  #append results
  varImportance_repetition<- as.data.frame(varImportance_repetition$importance)
  varImportance_repetition$varnames <- rownames(varImportance_repetition) # row names to column
  rownames(varImportance_repetition) <- NULL  
  varImportance <- merge(varImportance, varImportance_repetition, by ='varnames',all.x = TRUE,all.y = TRUE)
}

#get ranks
varRanks<-varImportance
for(i in 2:(maxIter+1)){
  varRanks[,i]<-order(order(-varImportance[,i]))
}

#get summary stats on results
repeated_importance_df <- data.frame('varnames'=varImportance$varnames,'Mean_Rel_Imp'=rowMeans(varImportance[,2:(maxIter+1)]),'Rel_Imp_Sd' = apply(varImportance[,2:(maxIter+1)], 1, sd), 'Mean_Rank' = rowMeans(varRanks[,2:(maxIter+1)]), 'Rank_Sd' = apply(varRanks[,2:(maxIter+1)], 1, sd), 'Rank_Min' =  apply(varRanks[,2:(maxIter+1)], 1, min), 'Rank_Max' =  apply(varRanks[,2:(maxIter+1)], 1, max))

#order this new df
repeated_importance_df<-repeated_importance_df[order(repeated_importance_df$Mean_Rank), ] 

#same plot as paper
ggplot(repeated_importance_df, aes(x=Mean_Rank, y=reorder(varnames,-Mean_Rank))) +   
  geom_point() +
  geom_errorbar(aes(xmin = Mean_Rank - Rank_Sd, xmax = Mean_Rank +Rank_Sd), width=0.1) +
  xlab("Importance Rank") +
  ylab("Feature Name") +
  geom_vline(xintercept = 1, linetype=2, color="red", size=0.5) +
  theme_minimal()+
...
p0<-plot(VaxPartial,main="Percent_One_Dose_As_Of_Nov_1",type="l",lwd=4,col="cyan3",xlab="% with >= 1 Dose",cex.axis=1.5,cex.lab=1.5,cex.main=1.5)

HospPartial<- partial(boost, pred.var = "Confidence_In_Hospitals", train=allData[,-20])
p3<- plot(HospPartial,type="line",main="Confidence_In_Hospitals",lwd=4,col="coral1",cex.axis=1.5,xlab="% w/ BMI >= 30",cex.lab=1.5,cex.main=1.5)

TrustPartial<- partial(boost, pred.var = "Trust_Covid_Advice_Govt", train=allData[,-20])
p1<- plot(TrustPartial,type="line",main="Trust_Covid_Advice_Govt",lwd=4,col="cyan3",cex.axis=1.5,xlab="% w/ BMI >= 30",cex.lab=1.5,cex.main=1.5)

GDPPartial<- partial(boost, pred.var = "GDP_Per_Capita", train=allData[,-20])
p2<-plot(GDPPartial,main="GDP_Per_Capita",type="l",lwd=4,col="coral1",cex.axis=1.5,xlab="log(GDP Per Capita)",cex.lab=1.5,cex.main=1.5)

ObesePartial<- partial(boost, pred.var = "Obese_Adult_Percentage", train=allData[,-20])
p5<- plot(ObesePartial,type="line",main="Obese_Adult_Percentage",lwd=4,col="coral1",cex.axis=1.5,xlab="% with BMI >= 30",cex.lab=1.5,cex.main=1.5)

AgePartial<- partial(boost, pred.var = "Age_65_Older_Percent", train=allData[,-20])
p4<- plot(AgePartial,type="line",main="Age_65_Older_Percentt",lwd=4,col="coral1",cex.axis=1.5,xlab="% Population Aged >= 65 Years",cex.lab=1.5,cex.main=1.5)
