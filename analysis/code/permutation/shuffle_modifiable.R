here::i_am("analysis/World/code/permutation_test/shuffle_modifiable.R")
library(here)
library(dplyr)
library(ggplot2)
library(glmnet)
...
################################
# OBSERVED Residual difference #
################################

# ####------------------------- intrisinc + modifiable model --------------------------####
# X_ = cbind(X[,index.IM], Y=Y, iso=iso)
# iso_ = X_$iso
...
# Y.hat = predict(boost, newdata = X_)
# 
# 
# #obtain all predictions made in cv process
# preds<-Y.hat #fit$pred$pred
# actual<-Y #fit$pred$obs
# 
# #NOW THAT WE HAVE INDIVIDUAL PREDICTIONS AND OBSERVED VALS, WE CAN CONVERT TO ORIGINAL SCALE#
# preds_untrsf <- preds^3
# actual_untrsf <- actual^3
# 
# rMSE_modif = sqrt(mean((actual_untrsf - preds_untrsf)^2))
# 
...
# ####------------------------- intrisinc only model --------------------------####
# X_ = cbind(X[,index.I], Y=Y, iso=iso)
# 
# # X_[,index.intrinsic] = X_[order(X_permute_list[[p]]), index.int] 
# iso_ = X_$iso
# X_ = X_ %>% select(-iso) # contains Y
# 
# 
...
# actual_untrsf <- actual^3
# 
# rMSE_intrin = sqrt(mean((actual_untrsf - preds_untrsf)^2))    
# 
# 
# 
